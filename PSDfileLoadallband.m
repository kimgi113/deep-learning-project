dataset = table();
pattern = 'de_movingAve'; %de_moving, de_LDS, psd_moving, psd_LDS
band = 5; % 1=Delta, 2=theta, 3=alpha, 4=beta, 5=gamma
...
for k = 1:filelength
    ...
    for i = 1:length(selectPersondata)
        ...
        bandselectdata = selectdata;
        newdataset = table({bandselectdata}, session1_label(i));
...
for k = 1:filelength
    ...
    for i = 1:length(selectPersondata)
        ...
        selectdata = [filelistStruct.(selectPersondata(i))];
        bandselectdata = selectdata;
        newdataset = table({bandselectdata}, session2_label(i));
...
for k = 1:filelength
    ...
    for i = 1:length(selectPersondata)
        ...
        selectdata = [filelistStruct.(selectPersondata(i))];
        bandselectdata = selectdata;
        newdataset = table({bandselectdata}, session3_label(i));
